...
(defcustom jde-java-environment-variables 
...
function displays an error message."
  ...
  (if jde-jdk
      (message "es verdad esto?")
    (let* (
	   ;; DANGEROUS WALLY,  comment, uncomment
	   ;; (jdk (assoc (car jde-jdk) jde-jdk-registry))
	   ;; (jdk-dir (cdr jdk))
	   (jdk "1.6.0.16")
	   (jdk-dir "/usr/lib/jvm/java-6-sun-1.6.0.16")
	   )
	(when (null jdk)
	  (error (format

...
(defun twit-set-auth (user pass)
   ...
   (let (
		 (old-http-storage          (assoc "twitter.com:80"  (symbol-value url-basic-auth-storage)))
		 ...
		 (auth-pair                 (cons "Twitter API"      (base64-encode-string (format "%s:%s" "" ""))))
		 )
...
error."
  ...
  (condition-case nil
      (let ((twit-pass-var 'twit-pass))
        (when (and (not (string= (symbol-value twit-pass-var) "")) ;cuando no estan vacios...

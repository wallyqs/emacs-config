  ...
  (let ((marker-start (search-backward "<script type=\"text/javascript\"" min t))
    ...
    (when marker-start
      (when (looking-at mumamo-javascript-tag-start-regex)
        ...
        (when (<= exc-start pos)
          ;; (cons (point) 'js2-mode)
          (list (point) 'javascript-mode '(html-mode))
          )
        ))))
